...
public class FragmentGalleryReviewImages extends FragmentGalleryActionStorage {
    ...
    protected void clickItem(int adapter, int position) {
        ...
        LYTE("FragmentGalleryReviewImages click item - "+position+", adapter "+getListFolds().get((getFoldAdapt().sortKeys()[adapter])));
//        ArrayList<String> imgs = getListImagesInFolders().get(getFoldAdapt().sortKeys()[adapter]);
