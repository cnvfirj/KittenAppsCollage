...
import com.kittendevelop.kittenappscollage.draw.operations.bitmap.CoercionBitmap;
import com.kittendevelop.kittenappscollage.draw.operations.bitmap.DrawBitmap;
...
//        }
    ...
    private void correctOpLyr(){
        if(isImg()){
            ...
            DeformMat mat = new DeformMat(App.getMain());
            mat.view(rView).bitmap(new PointF(temp.getWidth(), temp.getHeight())).getRepository().setScale(rLyrMat.getRepository().getScale());
            mat.getRepository().setTranslate(trans);
