...
public class DrawMaxiVeil extends DrawVeil {

    private RectF content;

    public DrawMaxiVeil(Context context) {
        super(context);
    }

    public DrawMaxiVeil(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
    }
...
//            if(getTarget().width()<getWidth()/4){
                if(getTarget().right+getIndent()<getWidth()/3||getTarget().left-getIndent()>getWidth()/3)
                    return maxiVeilInVerticalTarget();
//            }
        }
        return maxiVeilInHorisontalTarget();
    }
...
//            canvas.clipPath(transform(content));
    ...
    private RectF maxiVeilInVerticalTarget(){
        RectF r = new RectF();
    ...
    private RectF maxiVeilInHorisontalTarget(){
        RectF r = new RectF();
        RectF target = addFrameTarget();
        if (target.top > (getHeight() - target.bottom)) {
